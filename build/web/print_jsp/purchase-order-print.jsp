        ...
        <div style="padding-top: 120px; width: 100%;">
            ...
            <div style="clear: both;overflow: hidden; padding-top: 5px;">
                <div style="width: 65%; float: left; font-size: 12px;">
                    <p> Name & GSTIN No : ${requestScope.header.customeName}</p>
                </div>  
                <div style="width: 35%;float: left;">
            ...
            <div style="clear:both;overflow:hidden;">
                <table  border="1px" style="width:90%;text-align: center; font-size: 12px;">
                    <tr>
                        ...
                        <th>Discount</th>
                        <c:choose>
                            <c:when test="${requestScope.header.sgst eq 0 && requestScope.header.cgst eq 0}">
                                <th>VAT</th>
                            </c:when>
                        </c:choose>
                        <th>Amount</th>
                    </tr>
                    <c:set var="colspam" value="8"/>
                    ...
                    <c:choose>
                        <c:when test="${requestScope.header.sgst gt 0 && requestScope.header.cgst gt 0}">
                            <c:set var="colspam" value="7"/>
                            <tr>
                                <td colspan="${colspam}" style="pull-right"><label>SGST <fmt:formatNumber pattern="#0.00"  value="${requestScope.header.gstPercentage / 2 }"/> % </label></td> 
                                    <!--8-->
                                <td><fmt:formatNumber pattern="#0.00"  value="${requestScope.header.sgst}"/></td>
                            </tr>
                            <tr>
                                <td colspan="${colspam}"><label>CGST <fmt:formatNumber pattern="#0.00"  value="${requestScope.header.gstPercentage / 2 }"/> %  </label></td>
                                    <!--8-->
                                <td><fmt:formatNumber pattern="#0.00"  value="${requestScope.header.cgst}"/></td>
                            </tr>
                        </c:when>
                        <c:otherwise>
                             <c:set var="colspam" value="8"/>
                            <tr>
                                <td colspan="${colspam}" style="pull-right"><label>SGST </label></td> 
                                <td><fmt:formatNumber pattern="#0.00"  value="${requestScope.header.sgst}"/></td>
                            </tr>
                            <tr>
                                <td colspan="${colspam}"><label>CGST </label></td>
                                <td><fmt:formatNumber pattern="#0.00"  value="${requestScope.header.cgst}"/></td>
                            </tr>
                        </c:otherwise>
                    </c:choose>
                    <tr>
                        <td colspan="${colspam}"><label>Packing / Gift Charge </label></td>
                          <!--8-->
            ...
            <div style="clear: both;overflow: hidden; padding: 10px 0px 20px 0px;">
                ...
                <div style="width: 20%;float: right;">
                    <h5>Billed By</h5>
                    <h5 style="float:left;">${requestScope.header.empName}</h5>
                </div>
            </div>
           
                <p style="float: right; margin-left: -15px;">Manager / DGM</p>
                <p>HSN Code : ${requestScope.header.hsnCode}</p>
                <p><br>
                    P.S. :<br> 1. Goods once sold cannot be taken back or exchanged.<br>
                           2. The total tax collected apportioned to SGST & CGST 50% each.
                </p>
        </div>
